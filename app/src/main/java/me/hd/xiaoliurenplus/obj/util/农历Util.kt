...
object 农历Util {
    ...
    private val solarTerm = arrayListOf(
        "小寒", "大寒", "立春", "雨水", "惊蛰", "春分",
        "清明", "谷雨", "立夏", "小满", "芒种", "夏至",
        "小暑", "大暑", "立秋", "处暑", "白露", "秋分",
        "寒露", "霜降", "立冬", "小雪", "大雪", "冬至"
    ...
    private fun lYearDays(y: Int): Int {
        ...
        var i = 0x8000
        while (i > 0x8) {
            sum += if ((农历Util.lunarInfo[y - 1900] and i) != 农历Util.FALSE) 1 else 0
            i = i shr 1
        }
    ...
    private fun leapMonth(y: Int): Int {
        return 农历Util.lunarInfo[y - 1900] and 0xf
    }

    private fun leapDays(y: Int): Int {
        return if (leapMonth(y) != 农历Util.FALSE) {
            if ((农历Util.lunarInfo[y - 1900] and 0x10000) != 农历Util.FALSE) 30 else 29
        } else 0
    }
    ...
    private fun monthDays(y: Int, m: Int): Int {
        ...
        return if (((农历Util.lunarInfo[y - 1900] and (0x10000 shr m)) != 农历Util.FALSE)) 30 else 29
    }
    ...
    private fun solarDays(y: Int, m: Int): Int {
        ...
        return if (ms == 1) {
            ...
            农历Util.solarMonth[ms]
        }
    }

    private fun toGanZhi(offset: Int): String {
        return 农历Util.gan[offset % 10] + 农历Util.zhi[offset % 12]
    }
    ...
    private fun getTerm(y: Int, n: Int): Int {
        ...
        val table = 农历Util.sTermInfo[y - 1900]
    ...
    fun toChinaDay(d: Int): String {
        return when (d) {
            ...
            30 -> "\u4e09\u5341"
                ...
                var s = 农历Util.nStr2[floor((d / 10f)).toInt()]
                s += 农历Util.nStr1[d % 10]
                s
            }
        }
     ...
     * @return [农历Bean]
     */
    fun solar2lunar(y: Int, m: Int, d: Int): 农历Bean? {
        if (y < 1900 || y > 2100) {
            return null
        }
        if (y == 1900 && m == 1 && d < 31) {
        ...
        if (firstNode == d) {
            isTerm = true
            term = 农历Util.solarTerm[m * 2 - 2]
        }
        if (secondNode == d) {
            isTerm = true
            term = 农历Util.solarTerm[m * 2 - 1]
        }
     ...
     * @return [农历Bean]
     */
    fun lunar2solar(y: Int, m: Int, d: Int, isLeapMonth: Boolean = false): 农历Bean? {
