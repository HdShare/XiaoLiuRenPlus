...
import me.hd.xiaoliurenplus.obj.bean.农历Bean
import java.util.Calendar
import kotlin.math.floor

object 农历Util {
    private val 润大小信息表 = arrayListOf(
    ...
    private val 月份天数表 = arrayListOf(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)
    private val 天干表 = arrayListOf("甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸")
    private val 地支表 =
        arrayListOf("子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥")
    private val 生肖表 =
        arrayListOf("鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪")
    private val 节气表 = arrayListOf(
        "小寒", "大寒", "立春", "雨水", "惊蛰", "春分",
        "清明", "谷雨", "立夏", "小满", "芒种", "夏至",
        "小暑", "大暑", "立秋", "处暑", "白露", "秋分",
        "寒露", "霜降", "立冬", "小雪", "大雪", "冬至"
    )
    private val 节气日期表 = arrayListOf(
    ...
    private val 数字转中文表 =
        arrayListOf("日", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十")
    private val 日期转农历表 = arrayListOf("初", "十", "廿", "卅")
    private val 月份转农历表 =
        arrayListOf("正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊")

    private fun 取农历年总天数(y: Int): Int {
        var sum = 348
        var i = 0x8000
        while (i > 0x8) {
            sum += if ((润大小信息表[y - 1900] and i) != 0) 1 else 0
            i = i shr 1
        }
        return (sum + 取农历年闰月天数(y))
    }

    private fun 取农历年闰月份(y: Int): Int {
        return 润大小信息表[y - 1900] and 0xf
    }

    private fun 取农历年闰月天数(y: Int): Int {
        return if (取农历年闰月份(y) != 0) {
            if ((润大小信息表[y - 1900] and 0x10000) != 0) 30 else 29
        } else 0
    }

    private fun 取农历年非闰月天数(y: Int, m: Int): Int {
        if (m > 12 || m < 1) return -1
        return if ((润大小信息表[y - 1900] and (0x10000 shr m)) != 0) 30 else 29
    }

    private fun 取公历年月天数(y: Int, m: Int): Int {
        if (m > 12 || m < 1) return -1
        val ms = m - 1
        return if (ms == 1) {
    ...
    private fun 取干支(offset: Int): String {
        return 天干表[offset % 10] + 地支表[offset % 12]
    }

    private fun 取公历年N节气公历日期(y: Int, n: Int): Int {
        if (y < 1900 || y > 2100) return -1
        if (n < 1 || n > 24) return -1
        val table = 节气日期表[y - 1900]
    ...
    fun 取农历月份中文(m: Int): String {
        if (m > 12 || m < 1) return ""
        return 月份转农历表[m - 1] + "月"
    }

    fun 取农历日期中文(d: Int): String {
        return when (d) {
            10 -> "初十"
            20 -> "二十"
            30 -> "三十"
            else -> {
                日期转农历表[floor(d / 10f).toInt()] + 数字转中文表[d % 10]
            }
        }
    }

    fun 取生肖(y: Int): String {
        return 生肖表[(y - 4) % 12]
    }

    fun 公历转农历(y: Int, m: Int, d: Int): 农历Bean? {
        ...
        objDate.set(y, m - 1, d)
        var offset = (objDate.timeInMillis - baseDate.timeInMillis) / 86400000

        var iYear = 1900
        var daysOfYear = 0
        while (iYear < 2101 && offset > 0) {
            daysOfYear = 取农历年总天数(iYear)
            offset -= daysOfYear
            iYear++
        }
        if (offset < 0) {
        ...
        val cWeek = 数字转中文表[nWeek]
        if (nWeek == 0) nWeek = 7

        val year = iYear
        val leap = 取农历年闰月份(iYear)
        var isLeap = false
        var iMonth = 1
        var daysOfMonth = 0
        while (iMonth < 13 && offset > 0) {
            if (leap > 0 && iMonth == (leap + 1) && !isLeap) {
                --iMonth
                isLeap = true
                daysOfMonth = 取农历年闰月天数(year)
            } else {
                daysOfMonth = 取农历年非闰月天数(year, iMonth)
            }
        ...
        if (offset == 0L && leap > 0 && iMonth == leap + 1)
            if (isLeap) {
                isLeap = false
            } else {
                isLeap = true
        ...
        val firstNode = 取公历年N节气公历日期(y, m * 2 - 1)
        val secondNode = 取公历年N节气公历日期(y, m * 2)
        var gzM = 取干支((y - 1900) * 12 + m + 11)
        if (d >= firstNode) {
            gzM = 取干支((y - 1900) * 12 + m + 12)
        }
        ...
        val gzD = 取干支((dayCyclical + d - 1).toInt())

        return 农历Bean(
            year, month, day.toInt(),
            取生肖(year),
            (if (isLeap) "闰" else "") + 取农历月份中文(month),
            取农历日期中文(day.toInt()),
            y, m, d,
            gzY, gzM, gzD,
            isToday, isLeap,
    ...
    fun 农历转公历(y: Int, m: Int, d: Int, isLeapMonth: Boolean = false): 农历Bean? {
        val leapMonth = 取农历年闰月份(y)

        if (isLeapMonth && (leapMonth != m)) return null
        if (y == 2100 && m == 12 && d > 1 || y == 1900 && m == 1 && d < 31) return null

        val day = 取农历年非闰月天数(y, m)
